alter table public.sales
  ...
  add constraint sales_promotion_months_valid
    check (promotion_months is null or promotion_months between 3 and 6);
  ...
  if operator_is_claro then
    ...
    if coalesce(new.is_multi, false) then
      ...
      if coalesce(new.has_promotion, false) then
        if new.promotion_months is null or new.promotion_months < 3 or new.promotion_months > 6 then
          raise exception 'Informe a duração do desconto entre 3 e 6 meses.';
        end if;
        if new.post_promo_value is null or new.post_promo_value <= new.value then
          raise exception 'O Total após o desconto deve ser maior que o Total inicial.';
        end if;
      else
        new.has_promotion := false; new.promotion_months := null; new.post_promo_value := null;
      end if;
    else
      new.is_multi := false; new.mobile_value := null; new.value := new.internet_value;
      new.has_promotion := false; new.promotion_months := null; new.post_promo_value := null;
    end if;
  else
...
comment on column public.sales.promotion_months is 'Quantidade de meses do desconto inicial, entre 3 e 6, contados desde a instalação.';
comment on column public.sales.post_promo_value is 'Valor total mensal após o fim do desconto.';
